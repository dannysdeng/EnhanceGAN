...
function ATMG:updateGradInput(_tranformParams, _gradParams)
  ...
  if self.fullMode then
    ...
    if self.useScale then
      local gradInputScaleparams = self.gradInput:narrow(2,paramIndex-1,2)
      ...
      temp = torch.cmul(selectedOutput, selectedGradParams):sum(2)
      gradInputScaleparams:copy(temp)
