...
local conv_crop_index = 37
  ...
  -- local Weighted_Crop = nn.Sum(3,1,true)(Weighted_Crop0)
  ...
  -- local Weighted_Touch = nn.Sum(3,1,true)(Weighted_Touch0)
...
--    curveA = nn.HardTanh()(nn.Narrow(2,3,1)(Weighted_Touch))
   curveA_const_k = (0.4 - 0.101) / 2; -- 0.15
...
netG = torch.load(string.format('./checkpoints/%s_%d_net_G.t7', opt.name, 2400))
collectgarbage()



netG:get(1):get(conv_touch_index):apply(weights_init)

netG:get(1):get(alphaA_output_index-1).constant_scalar = alpha_const_k
netG:get(1):get(alphaA_output_index).constant_scalar = alpha_const_b

netG:get(1):get(curveA_output_index-1).constant_scalar = curveA_const_k
netG:get(1):get(curveA_output_index).constant_scalar = curveA_const_b

netG:get(1):get(curveB_output_index-1).constant_scalar = curveB_const_k
netG:get(1):get(curveB_output_index).constant_scalar = curveB_const_b

netG:get(1):get(curveP1_output_index-1).constant_scalar = curveP1_const_k
netG:get(1):get(curveP1_output_index).constant_scalar = curveP1_const_b

netG:get(1):get(curveP2_output_index-1).constant_scalar = curveP2_const_k
netG:get(1):get(curveP2_output_index).constant_scalar = curveP2_const_b


-- netG:get(1):get(21):apply(weights_init)
-- netG:get(1):get(23):apply(weights_init)
...
model_copy:get(1):get(conv_crop_index):apply(function(m) 
  if m.weight then
     m.weight:fill(0)
  end
  if m.bias then
...
-- model_copy:get(1):get(alphaA_grad_index-1):apply(function(m) 
--   if m.weight then
--      m.weight:fill(0)
--   end
--   if m.bias then
...
-- end
-- )
-- model_copy:get(1):get(contrast_grad_index-1):apply(function(m) 
--   if m.weight then
--      m.weight:fill(0)
--   end
--   if m.bias then
...
-- end
-- )
-- model_copy:get(1):get(brightness_grad_index-1):apply(function(m) 
--   if m.weight then
--      m.weight:fill(0)
--   end
--   if m.bias then
...
local netD = torch.load(string.format('./checkpoints/%s_%d_net_D.t7', opt.name, 2400))
...
Lab2RGB_module = torch.load('./pretrained_ResNet/Lab2RGB_50_net_G.t7')
...
Lab2RGB_FULL:cuda()
perceptual_loss_net = torch.load('./pretrained_ResNet/vgg16.t7')
...
for epoch = 1, opt.niter do
   ...
   while i <= math.min(data:size(), opt.ntrain) do
      ...
      local absERROR = torch.abs(errD_real[1] - errG[1])
      if absERROR < 1 or generation_iter == 0 or generation_iter == 2400 or errD_real[1] > errG[1] then 
      ...
      -- logging
       ...
       if generation_iter % 200 == 0 then
          ...
          netG:training()
          netD:training()
           -- o1 = netG:forward(torch.CudaTensor(8,3,224,224):uniform());
           -- b1 = netG:backward(torch.CudaTensor(8,3,224,224):uniform(), o1:fill(1))
           print(string.format('Size of parameters after saving'))
           print(parametersG:size())
           print(gradParametersG:size())
      end    
      if generation_iter >= 3800 then
        print('Finished stage 2. Terminating...')
        return
      end     
