...
function dataset:getByClass(class)
   ...
   local k = torch.randperm(5)[1]
   local imgID_KNN = self.KNN_Matrix[index][k]
   local imgpah2 = string.format('../train_data/myimages_good_MultiWhole/1/%d.jpg', imgID_KNN)

   return self:sampleHookTrain(imgpath, imgpah2)
end
...
local function tableToOutput(self, dataTable, scalarTable)
   ...
   data = torch.Tensor(quantity,
		       6, self.sampleSize[2], self.sampleSize[3])
